...
**问题**：粘贴含换行的内容（报错信息、代码片段）时，每个 `\n` 被当成 Enter 直接提交，无法完整粘贴多行文本。即使改了这个，多行内容也全被塞进一行显示框，加上 `↵` 标记后撑爆边框，UI 完全乱掉。

**根本原因（两个）**：
1. 输入循环将 `\r`/`\n` 无条件视为提交，没有区分"手动按 Enter"和"粘贴中的换行"。
2. `redraw()` 始终按单行绘制，内容超出终端宽度时边框和内容混在一起。

**解决方案**：
1. **Bracketed Paste Mode**：Readline 开始时发送 `\033[?2004h` 启用，退出时 `\033[?2004l` 关闭。终端会在粘贴内容前后包裹 `\033[200~` / `\033[201~`，作为 `runeStartPaste`/`runeEndPaste` 哨兵处理。粘贴期间 `inPaste=true`，此时 `\r`/`\n` 作为字面换行插入缓冲区而非提交。
2. **输入框自适应高度**：`redraw()` 按 `\n` 分割缓冲区，每行单独绘制一行，框高动态扩展（顶部边框 + N 行内容 + 底部边框）。新增 `boxCursorAbove` 记录光标到顶部边框的行数，用于下次重绘时精确定位。`clearBox()` 同步更新，显示首行 + `[+N lines]` 摘要。
3. **历史记录兼容**：多行条目在写入历史文件时将 `\n` 转义为 `\x01`，加载时还原，不破坏原有格式。
